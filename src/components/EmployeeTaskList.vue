<script setup lang="ts">
   ...
   const seeAllTasks = ref<boolean>(false)
   ...
   const setMyTasks = () => {
      pendingTasks.value = tasks.value.filter(t => t.status === PENDING && t.user.id === userId)
      completedTasks.value = tasks.value.filter(t => t.status === COMPLETED && t.user.id === userId)
      progressTasks.value = tasks.value.filter(t => t.status === PROGRESS && t.user.id === userId)
      blockedTasks.value = tasks.value.filter(t => t.status === BLOCKED && t.user.id === userId)
   }
   ...
   <div class="flex justify-between items-center">
      ...
      <div class="form-control">
         <label class="label cursor-pointer space-x-5">
            <span class="label-text">Ver todas las Tareas</span> 
            <input type="checkbox" class="checkbox" v-model="seeAllTasks" />
         </label>
      </div>
   </div>
